          ...
          <div class="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden dark:bg-neutral-900 dark:border-neutral-700">
                  ...
                  <div class="inline-flex gap-x-2">
                    <form action="{{ route('users.index') }}" method="get">
                        <div class="max-w-sm space-y-3">
                            <input value="{{ request('search') }}" type="text" name="search" class="py-3 px-5 block w-full border-gray-200 rounded-full text-sm focus:border-blue-500 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none dark:bg-neutral-900 dark:border-neutral-700 dark:text-neutral-400 dark:placeholder-neutral-500 dark:focus:ring-neutral-600" placeholder="Input text">
                          </div>
                    </form>
            ...
            <table class="min-w-full divide-y divide-gray-200 dark:divide-neutral-700">
              ...
              <tbody class="divide-y divide-gray-200 dark:divide-neutral-700">
                ...
                <tr>
                  <td class="size-px whitespace-nowrap px-6 py-3">
                    <button type="button" class="flex items-center gap-x-2 text-gray-800 hover:text-gray-600 focus:outline-none focus:text-gray-600 dark:text-neutral-200 dark:hover:text-neutral-400 dark:focus:text-neutral-400">
                      <span class="text-sm text-gray-800 dark:text-neutral-200">{{$index + $users->firstItem() }}</span>
                    </button>
                  </td>
                  <td class="size-px whitespace-nowrap px-6 py-3">
                      <img class="inline-block size-[38px] rounded-full" src="{{ asset('storage/users/'.$user->photo_profile) }}" alt="Avatar">
                      <span class="font-semibold text-sm text-gray-800 dark:text-white">{{ $user->name }}</span>
                  </td>
                  <td class="size-px whitespace-nowrap px-6 py-3">
                    @foreach ($user->roles as $role)
                        <span class="text-sm text-gray-800 dark:text-neutral-200">{{ $role->name }}</span>
                    @endforeach
                  </td>
                  <td class="size-px whitespace-nowrap px-6 py-3">
                  ...
                  <td class="size-px whitespace-nowrap px-6 py-3">
                    <span class="text-sm text-gray-800 dark:text-neutral-200">{{ $user->address ?? '--' }}</span>
                  </td>
                  <td class="size-px whitespace-nowrap">
                    <div class="px-6 py-1.5 flex gap-2">
                      <a href="{{ route('users.edit', $user->id) }}" class="inline-flex items-center gap-x-1 text-sm text-blue-600 decoration-2 hover:underline focus:outline-none focus:underline font-medium dark:text-blue-500">
                        Edit
                      </a>
                ...
                @empty
                <h class="mt-4 text-center dark:text-gray-300 text-2xl">No Data</h>
                @endforelse
            </tbody>
        </table>
